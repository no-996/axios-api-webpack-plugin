...
function parse(cfg, parent = [], pName = '') {
  if (cfg) {
    ...
    parent.push(`/**\n ${parseComment((cfg.name || 'empty') + (cfg.des ? ' - ' + cfg.des : ''))}${parseComment(cfg.url ? `(${cfg.method || 'get'}) ${'url: ' + cfg.url}` : '', true)} */`);
    parent.push(`interface ${parseName(cfg)}Instance {`);

    if (cfg.children) {
      cfg.children.forEach(o => {
        parent.push(`/**\n ${parseComment((o.name || 'empty') + (o.des ? ' - ' + o.des : ''))}${parseComment(o.url ? `(${o.method || 'get'}) ${'url: ' + o.url}` : '', true)} */`);
