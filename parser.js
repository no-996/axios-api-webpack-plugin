...
function parseRequestOptionsDefaultData(opts, metadata = {}, root = true, level = 0, lines = []) {
  ...
  for (let p in opts) {
    ...
    const comment = (_metadata$p2 = metadata[p]) !== null && _metadata$p2 !== void 0 && _metadata$p2.name ? `// ${(_metadata$p3 = metadata[p]) === null || _metadata$p3 === void 0 ? void 0 : _metadata$p3.name}${des ? ' - ' + des : ''}` : '';
    ...
    if (Array.isArray(opts[p])) {
      ...
      opts[p].forEach(o => {
        if (typeof o === 'object') {
          ...
          lines.push(' * ' + blank3 + parseRequestOptionsDefaultData(o, typeof ((_metadata$p4 = metadata[p]) === null || _metadata$p4 === void 0 ? void 0 : _metadata$p4.type) === 'object' ? metadata[p].type : metadata[p], false, level + 2));
        } else {
      ...
      lines.push(' * ' + blank2 + `"${p}": ${parseRequestOptionsDefaultData(opts[p], typeof ((_metadata$p5 = metadata[p]) === null || _metadata$p5 === void 0 ? void 0 : _metadata$p5.type) === 'object' ? metadata[p].type : metadata[p], false, level + 1)}, ${comment}`);
...
function parse(cfg, parent = [], pName = '') {
  if (cfg) {
    const prefix = pName.split('.').filter(o => o) // 去除最后一级空
    .slice(1) // 去除Root
    .map(o => o[0].toUpperCase() + o.substring(1)) // 首字母大写
    .join('');
    parent.push(`\n/* ${new Array(25).fill('↓').join('')} ${pName}${cfg.name || 'empty'} ${new Array(25).fill('↓').join('')} */\n`);
    parent.push(`/**\n ${parseComment((cfg.name || 'empty') + (cfg.des ? ' - ' + cfg.des : ''))}${parseComment(cfg.url ? `(${cfg.method || 'get'}) ${'url: ' + cfg.url}` : '', true)} */`); // 增加父级

    parent.push(`interface ${prefix}${parseName(cfg)}Instance {`);

    if (cfg.children) {
      cfg.children.forEach(o => {
        parent.push(`/**\n ${parseComment((o.name || 'empty') + (o.des ? ' - ' + o.des : ''))}${parseComment(o.url ? `(${o.method || 'get'}) ${'url: ' + o.url}` : '', true)} */`); // 增加父级
